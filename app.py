...
if arq:
    ...
    #Separa a hora do início da ocorrência
    df['time'] = df['Hora da ocorrência'].str.split().apply(pd.Series)
    df['datetime'] = pd.to_datetime(df['time'])
    df['Hora_Início'] = df['datetime'].dt.hour
    df['Minuto_Início'] = df['datetime'].dt.minute
    df['Segundo_Início'] = df['datetime'].dt.second
    df = df.drop(['time', 'Segundo_Início', 'datetime'], 1)

    #Separa a hora do fim da ocorrência
    df['time'] = df['Fim da ocorrência'].str.split().apply(pd.Series)
    df['datetime'] = pd.to_datetime(df['time'])
    df['Hora_Fim'] = df['datetime'].dt.hour
    df['Minuto_Fim'] = df['datetime'].dt.minute
    df['Segundo_Fim'] = df['datetime'].dt.second
    df = df.drop(['time', 'Segundo_Fim', 'datetime'], 1)
